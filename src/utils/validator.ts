...
import { ProjectConfig, ValidationResult } from '../types/index.js';
import { normalizeEnvName, findFirst } from './file-utils.js';
import { parseExistingApplicationIds } from '../android/gradle-parser.js';

export function validate(config: ProjectConfig, projectRoot: string): ValidationResult {
  const errors: string[] = [];
  ...
  if (uniqueIds.size !== bundleIds.length) {
    errors.push(
      `Duplicate bundle IDs detected: ${bundleIds.filter((id, i) => bundleIds.indexOf(id) !== i).join(', ')}`
    );
  }
  ...
  if (fs.existsSync(gradlePath)) {
    ...
    for (const env of config.environments) {
      if (existingIds.includes(env.bundleId) && !gradleContent.includes('rn-env-setup')) {
        warnings.push(
          `Bundle ID "${env.bundleId}" already exists in build.gradle (not from rn-env-setup). It will be overwritten.`
        );
      }
    }
  }
  ...
  if (fs.existsSync(pkgPath)) {
    ...
    if (config.jsEnvStrategy === 'react-native-config' && !deps['react-native-config']) {
      warnings.push(
        'jsEnvStrategy is "react-native-config" but react-native-config is not installed. Run: yarn add react-native-config'
      );
    }
  }
